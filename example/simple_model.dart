/// Simple model example — minimum viable Odoo module.
/// Run with: flutter run example/simple_model.dart
library example_simple;
...
void main() => runApp(const SimpleModelApp());

// ════════════════════════════════════════════════════════════════════════════
//  APP
// ════════════════════════════════════════════════════════════════════════════

class SimpleModelApp extends StatelessWidget {
  const SimpleModelApp({super.key});
  @override
  Widget build(BuildContext context) => MaterialApp(
        title: 'Simple Model — Odoo Generator',
        debugShowCheckedModeBanner: false,
        theme: OdooTheme.dark(const Color(0xFF9C6ADE)),
        home: const SimpleModelPage(),
      );
}
...
class SimpleModelPage extends StatefulWidget {
  const SimpleModelPage({super.key});
  @override
  State<SimpleModelPage> createState() => _SimpleModelPageState();
}

class _SimpleModelPageState extends State<SimpleModelPage>
  ...
  Future<void> _generate() async {
    try {
      final model = odoo.OdooModel('x_product_custom',
          docstring: 'Custom E-Commerce Product',
          createTimestamp: true,
          order: 'name asc')
        ..dependency('product')
        ..dependency('stock')
        ..field(const odoo.NameField())
        ..field(const odoo.Many2one('supplier_id', relation: 'res.partner',
            inTree: true))
        ..field(const odoo.Many2one('category_id',
            relation: 'product.category'))
        ..field(const odoo.Float('price', digits: (10, 2)))
        ..field(const odoo.Float('cost', digits: (10, 2)))
        ..field(const odoo.Integer('quantity'))
        ..field(const odoo.Integer('min_qty'))
        ..field(const odoo.Text('description'))
        ..field(const odoo.Image('main_image'))
        ..field(const odoo.Boolean('active', defaultValue: 'True'))
        ..field(const odoo.Boolean('is_published'))
        ..compute(const odoo.ComputeDefinition(
          methodName: '_compute_margin',
          dependsOn: ['price', 'cost'],
          bodyLines: [
            'for p in self:',
            '    p.margin = (p.price - p.cost) / p.price * 100 if p.price else 0',
          ],
        ))
        ..field(const odoo.ComputedField('margin',
            fieldType: 'Float',
      ...
      final gen = odoo.OdooGenerator(model,
          outputPath: '/tmp/x_product_custom',
  ...
  Widget build(BuildContext context) => Scaffold(
        ...
        appBar: OdooAppBar(
          modelId: 'x_product_custom',
          title: 'Simple Model',
          accent: _accent,
          files: _files,
          onExport: _files == null ? null : () => ZipExporter.download(_files!, 'x_product_custom'),
        ),
        body: _loading
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        color: OdooTheme.gutter,
                        padding:
                            const EdgeInsets.fromLTRB(0, 14, 10, 14),
                        child: Column(
                      ...
                      // thin separator
                      Container(
                          width: 1, color: OdooTheme.border),
                      // code
                      Padding(
                        padding:
                            const EdgeInsets.fromLTRB(16, 14, 32, 28),
                        child: isXml
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Row(children: [
          ...
          Padding(
            ...
            child: _CopyBtn(
                copied: copied, accent: accent, onCopy: onCopy),
          ),
        ]),
      );
...
class PythonView extends StatelessWidget {
  ...
  static const _kw    = Color(0xFF569CD6); // keywords
  static const _builtin= Color(0xFF4EC9B0);// built-ins / class names
  static const _str   = Color(0xFFCE9178); // strings
  static const _num   = Color(0xFFB5CEA8); // numbers
  static const _cmt   = Color(0xFF6A9955); // comments
  static const _dec   = Color(0xFFDCDCAA); // decorators / functions
  static const _op    = Color(0xFFD4D4D4); // operators / default text
  static const _self  = Color(0xFF9CDCFE); // self / params
  static const _attr  = Color(0xFF4FC1FF); // attribute names
  ...
  List<TextSpan> _tokenize(String src) {
    ...
    for (var li = 0; li < lines.length; li++) {
      final line = lines[li];
      spans.addAll(_tokenizeLine(line));
  ...
  List<TextSpan> _tokenizeLine(String line) {
    ...
    for (final m in pattern.allMatches(line)) {
      ...
      if (m.group(1) != null) {
        ...
        if (_keywords.contains(tok)) {
          ...
          c = _self;
        } else if (tok[0] == tok[0].toUpperCase() && tok.length > 1) {
          c = _builtin; // PascalCase → type
    ...
    if (inString) {} // unused, suppress lint
    return spans;
  }
}
...
class XmlView extends StatelessWidget {
  ...
  List<TextSpan> _tokenize(String src) {
    ...
    for (final m in pat.allMatches(src)) {
      if (m.group(1) != null) {
        spans.add(TextSpan(
            text: m.group(1),
            style: const TextStyle(color: _cmt)));
      } else if (m.group(2) != null) {
  ...
  List<TextSpan> _tokenizeTag(String tag) {
    ...
    for (final m in pat.allMatches(tag)) {
      ...
      if (m.group(1) != null) {
        spans.add(TextSpan(
            text: tok, style: const TextStyle(color: _bracket)));
      } else if (m.group(2) != null) {
        ...
        spans.add(TextSpan(
            text: tok, style: const TextStyle(color: _str)));
      } else {
        spans.add(TextSpan(
            text: tok, style: const TextStyle(color: _text)));
      }
    }
    return spans;
  }
  ...
  Widget build(BuildContext context) => Center(
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Text('Running validators & templates',
              style: TextStyle(
                  color: OdooTheme.dimText, fontSize: 12)),
        ]),
      );
}
          ...
          child: Row(mainAxisSize: MainAxisSize.min, children: [
            ...
            Text(copied ? 'Copied!' : 'Copy',
                style: TextStyle(
                    fontSize: 11,
                    color:
                        copied ? accent : OdooTheme.mutedText)),
          ]),
        ),
      );
}
