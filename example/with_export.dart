/// Export example — manufacturing work order with ZIP export.
/// Run with: flutter run example/with_export.dart
library example_with_export;
...
void main() => runApp(const WithExportApp());

// ════════════════════════════════════════════════════════════════════════════
//  APP
// ════════════════════════════════════════════════════════════════════════════

class WithExportApp extends StatelessWidget {
  const WithExportApp({super.key});
  @override
  Widget build(BuildContext context) => MaterialApp(
        title: 'Work Order Export — Odoo Generator',
        debugShowCheckedModeBanner: false,
        theme: OdooTheme.dark(const Color(0xFF81C784)),
        home: const WithExportPage(),
      );
}
...
class WithExportPage extends StatefulWidget {
  const WithExportPage({super.key});
  @override
  State<WithExportPage> createState() => _WithExportPageState();
}

class _WithExportPageState extends State<WithExportPage>
  ...
  static const _accent = Color(0xFF81C784);
  ...
  Future<void> _generate() async {
    try {
      // x_work_order: manufacturing work order model
      final model = odoo.OdooModel('x_work_order',
          docstring: 'Manufacturing Work Order',
        ...
        ..field(const odoo.Many2one('product_id',
            relation: 'product.product', required: true, inTree: true))
        ..field(const odoo.Many2one('workcenter_id',
            relation: 'mrp.workcenter', required: true, inTree: true))
        ..field(const odoo.Many2one('production_id',
            relation: 'mrp.production'))
        ..field(const odoo.Many2one('operator_id',
            relation: 'hr.employee', inTree: true))
        ..field(const odoo.Many2one('supervisor_id',
        ...
        ..field(odoo.Date('scheduled_date'))
        ..field(odoo.Date('start_date'))
        ..field(odoo.Date('end_date'))
        ..field(const odoo.Float('qty_production', digits: (10, 2)))
        ..field(const odoo.Float('qty_produced', digits: (10, 2)))
        ..field(const odoo.Float('duration_expected', digits: (8, 2)))
        ..field(const odoo.Float('duration_actual', digits: (8, 2)))
        ..field(const odoo.Integer('sequence'))
        ..field(const odoo.Text('work_instructions'))
        ..field(const odoo.Text('quality_notes'))
        ..field(const odoo.Boolean('active', defaultValue: 'True'))
        ..field(const odoo.Boolean('is_rework'))
        ..field(const odoo.Many2many('component_ids', relation: 'product.product'))
        // compute: qty remaining
        ..compute(const odoo.ComputeDefinition(
          methodName: '_compute_qty_remaining',
          dependsOn: ['qty_production', 'qty_produced'],
          bodyLines: [
            'for wo in self:',
            '    wo.qty_remaining = max(0.0, wo.qty_production - wo.qty_produced)',
          ],
        ))
        ..field(const odoo.ComputedField('qty_remaining',
            fieldType: 'Float',
            computeMethod: '_compute_qty_remaining',
            store: true,
            depends: ['qty_production', 'qty_produced']))
        // compute: efficiency
        ..compute(const odoo.ComputeDefinition(
          methodName: '_compute_efficiency',
          dependsOn: ['duration_expected', 'duration_actual'],
          bodyLines: [
            'for wo in self:',
            '    if wo.duration_actual > 0:',
            '        wo.efficiency = wo.duration_expected / wo.duration_actual * 100',
            '    else:',
            '        wo.efficiency = 0.0',
          ],
        ))
        ..field(const odoo.ComputedField('efficiency',
            fieldType: 'Float',
            computeMethod: '_compute_efficiency',
            store: true,
            depends: ['duration_expected', 'duration_actual']))
        // compute: completion percentage
        ..compute(const odoo.ComputeDefinition(
          methodName: '_compute_completion',
          dependsOn: ['qty_production', 'qty_produced'],
          bodyLines: [
            'for wo in self:',
            '    if wo.qty_production > 0:',
            '        wo.completion_pct = wo.qty_produced / wo.qty_production * 100',
            '    else:',
            '        wo.completion_pct = 0.0',
          ],
        ))
        ..field(const odoo.ComputedField('completion_pct',
            fieldType: 'Float',
            computeMethod: '_compute_completion',
            store: true,
            depends: ['qty_production', 'qty_produced']))
        ..sqlConstraint('positive_qty',
            'CHECK(qty_production > 0)', 'Production quantity must be positive')
        ..sqlConstraint('valid_produced',
            'CHECK(qty_produced >= 0)', 'Produced quantity cannot be negative')
        ..sqlConstraint('valid_duration',
            'CHECK(duration_expected >= 0)', 'Expected duration cannot be negative')
        ..sqlConstraint('unique_name', 'UNIQUE(name)', 'Work order number must be unique');

      final gen = odoo.OdooGenerator(model,
          outputPath: '/tmp/x_work_order',
  ...
  Widget build(BuildContext context) => Scaffold(
        ...
        appBar: OdooAppBar(
          modelId: 'x_work_order',
          title: 'Work Order Export',
          accent: _accent,
          files: _files,
          onExport: _files == null
              ? null
              : () => ZipExporter.download(_files!, 'x_work_order'),
        ),
        body: _loading
